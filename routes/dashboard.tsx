...
export const handler = define.handlers({
  async GET(ctx) {
    ...
    const { amount, currentTime } = parseAbacParams(
      new URL(ctx.req.url).searchParams,
    );
    const results = await batchCheck(
      buildItems(persona.id, user, { amount, currentTime }),
    );
...
function ActionItems({ r, results, user, personaId, abacOpts }: CardProps) {
  return (
        ...
        <li key={a.key} class="flex items-center justify-between gap-3">
          <div class="min-w-0">
            <div class="truncate text-sm text-slate-200">
              {a.label}
              {a.abac === "withdraw" && (
                <span class="ml-1 font-medium text-amber-300">
                  · {abacOpts.amount}g
                </span>
              )}
            </div>
          ...
          <Badge
            ...
            relation={a.relation}
            object={r.object}
            context={abacContext(a.abac, abacOpts)}
            concept={a.concept}
          />
        </li>
      ))}
...
function ResourceCard({ r, results, user, personaId, abacOpts }: CardProps) {
  return (
...
function VaultPanel(
  { vault, tabs, results, user, personaId, abacOpts }: {
    vault: Resource;
    tabs: Resource[];
    results: Record<string, boolean>;
    user: string;
...
export default define.page<typeof handler>(function Dashboard({ data, state }) {
  const { results, allowed, total, amount, currentTime } = data;
  const persona = state.persona;
  const user = state.user;
  const personaId = persona.id;
  const abacOpts = { amount, currentTime };
  ...
  return (
    <div class="space-y-6">
      ...
      <p class="text-center text-xs text-slate-500">
        ...
        = ABAC result under the context set at the top of this page. Tweak the
        withdrawal amount and signup window there, or compare every persona on
        the{" "}
